...
export const renderMixedContent = (text) => {
  ...
  const tieneDelimitadores =
    text.includes("$$") || /(?<!\$)\$(?!\$)/.test(text); // $ pero no $$
  ...
  if (tieneComandosLatex && !tieneDelimitadores) {
    ...
    const gatheredMatch = latexLimpio.match(
      /\\begin\{gathered\}([\s\S]*?)\\end\{gathered\}/,
    );
    if (gatheredMatch) {
    ...
    if (latexLimpio.includes("\\end{gathered}")) {
      const idxEndGathered =
        latexLimpio.indexOf("\\end{gathered}") + "\\end{gathered}".length;
      // Solo quitar si hay menos de 50 caracteres después (probablemente duplicado)
      if (
        idxEndGathered < latexLimpio.length &&
        latexLimpio.length - idxEndGathered < 200
      ) {
    ...
    const yaEnvuelto =
      latexLimpio.includes("\\begin{gathered}") ||
    ...
    const noTieneAmbienteMultilinea =
      !latexLimpio.includes("\\begin{gathered}") &&
      !latexLimpio.includes("\\begin{aligned}") &&
      !latexLimpio.includes("\\begin{array}") &&
      !latexLimpio.includes("\\begin{cases}") &&
