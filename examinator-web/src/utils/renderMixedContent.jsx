...
const ColorSwatch = ({ hex }) => {
  ...
  if (!rgb) return <span style={{ color: '#ef4444' }}>⚠️ Color inválido: {hex}</span>;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span style={{ color: '#60a5fa', fontWeight: '600', width: '50px' }}>HEX:</span>
          <code style={{ color: '#e2e8f0', fontFamily: 'monospace', background: 'rgba(59, 130, 246, 0.15)', padding: '2px 8px', borderRadius: '4px' }}>{hex}</code>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span style={{ color: '#10b981', fontWeight: '600', width: '50px' }}>RGB:</span>
          <code style={{ color: '#e2e8f0', fontFamily: 'monospace', background: 'rgba(16, 185, 129, 0.15)', padding: '2px 8px', borderRadius: '4px' }}>
            rgb({rgb.r}, {rgb.g}, {rgb.b})
          </code>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span style={{ color: '#8b5cf6', fontWeight: '600', width: '50px' }}>RGBA:</span>
          <code style={{ color: '#e2e8f0', fontFamily: 'monospace', background: 'rgba(139, 92, 246, 0.15)', padding: '2px 8px', borderRadius: '4px' }}>
            rgba({rgb.r}, {rgb.g}, {rgb.b}, 1)
          </code>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          <span style={{ color: '#f59e0b', fontWeight: '600', width: '50px' }}>CMYK:</span>
          <code style={{ color: '#e2e8f0', fontFamily: 'monospace', background: 'rgba(245, 158, 11, 0.15)', padding: '2px 8px', borderRadius: '4px' }}>
...
const ColorCombination = ({ colors }) => {
  return (
    <div style={{
      ...
      <span style={{ color: '#a78bfa', fontWeight: '600', marginRight: '0.5rem' }}>🎨 Combinación:</span>
      {colors.map((hex, idx) => {
        const rgb = hexToRgb(hex);
        return (
          <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
            ...
            <code style={{ color: '#e2e8f0', fontSize: '0.65rem', fontFamily: 'monospace' }}>{hex}</code>
            {rgb && (
...
const ColorMix = ({ colors, result }) => {
  return (
    <div style={{
      ...
      <span style={{ color: '#34d399', fontWeight: '600', marginRight: '0.25rem' }}>🧪 Mezcla:</span>
      ...
      {colors.map((hex, idx) => (
        <div key={idx} style={{ display: 'flex', alignItems: 'center' }}>
          ...
          {idx < colors.length - 1 && (
            <span style={{ color: '#94a3b8', margin: '0 0.35rem', fontSize: '1rem', fontWeight: 'bold' }}>+</span>
          )}
        </div>
      ))}
      
      {/* Flecha */}
      <span style={{ color: '#6ee7b7', margin: '0 0.5rem', fontSize: '1.2rem' }}>→</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
        ...
        <code style={{ color: '#a7f3d0', fontSize: '0.75rem', fontFamily: 'monospace' }}>{result}</code>
      </div>
    </div>
  );
};
...
export const renderMixedContent = (text) => {
  ...
  // 4. Display math: $$...$$
  // 5. Inline math: $...$
  const combinedRegex = /\[COLORMIX:((?:#[a-fA-F0-9]{6}\+?)+)=(#[a-fA-F0-9]{6})\]|\[COLORS:((?:#[a-fA-F0-9]{6}\+?)+)\]|\[COLOR:(#[a-fA-F0-9]{6}|#[a-fA-F0-9]{3})\]|\$\$([^\$]+)\$\$|\$([^\$]+)\$/g;
  let match;
  ...
  return (
    <div style={{
      ...
      {lineas.map((linea, lineaIdx) => {
        if (!linea.trim()) return <div key={lineaIdx} style={{ height: '0.5rem' }} />;
        ...
        if (hasColors) {
          ...
          const titulo = primerIndex > 0 ? linea.slice(0, primerIndex).trim() : '';
          ...
          return (
            <div key={lineaIdx} style={{
              ...
              <div style={{
                ...
                {colores.map((hex, idx) => {
                  const rgb = hexToRgb(hex);
                  const isNeon = ['#ff00ff', '#00ffff', '#ffff00', '#ff1493', '#00ff00', '#ff0000', '#00ff7f', '#ff6b00'].some(
                    n => n.toLowerCase() === hex.toLowerCase()
                  ) || (rgb && (rgb.r > 200 || rgb.g > 200 || rgb.b > 200) && (rgb.r === 0 || rgb.g === 0 || rgb.b === 0 || rgb.r === 255 || rgb.g === 255 || rgb.b === 255));
