...
const ArtToolbar = ({ onInsertElement }) => {
  ...
  const parseColorInput = (input, format) => {
    try {
      if (format === 'hex') {
        ...
        const match = input.match(/(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/);
        if (match) {
          const c = parseInt(match[1]), m = parseInt(match[2]), y = parseInt(match[3]), k = parseInt(match[4]);
          const rgb = cmykToRgb(c, m, y, k);
          return { hex: rgbToHex(rgb.r, rgb.g, rgb.b), rgb, cmyk: { c, m, y, k } };
        }
      }
    } catch (e) {
  ...
  const quitarColorDeCombi = (hexToRemove) => {
    setColoresSeleccionados(coloresSeleccionados.filter(c => c !== hexToRemove));
  };
    ...
    <div style={{ width: '100%' }}>
      ...
      {activeTab === 'editor' && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ color: '#e9d5ff', margin: '0 0 0.5rem 0', fontSize: '1rem' }}>
              ✏️ Editor de Figuras Geométricas
            </h3>
            <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
              Crea composiciones con círculos, cuadrados, triángulos, líneas y más.
              Haz clic en el canvas blanco para agregar figuras, selecciónalas para editar su tamaño y color.
            </p>
          </div>
          <ShapeEditor 
            onInsert={(texto) => onInsertElement(texto)}
          />
        </div>
      )}
      ...
      {activeTab === 'colores' && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: '200px' }}>
              <label style={{ display: 'block', color: '#94a3b8', fontSize: '0.8rem', marginBottom: '0.5rem' }}>
                ...
                {colorFormat === 'rgba' && 'Introduce RGBA (ej: 255, 87, 51, 0.8)'}
              ...
              <input
                ...
                placeholder={
                  ...
                  colorFormat === 'rgb' ? '255, 87, 51' :
                  colorFormat === 'rgba' ? '255, 87, 51, 0.8' :
                  '0, 66, 80, 0'
                }
                style={{
                  width: '100%',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem' }}>
              <label style={{ color: '#94a3b8', fontSize: '0.75rem' }}>Selector</label>
              <input
                type="color"
          ...
          {currentColor && (
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: '200px' }}>
                  <h4 style={{ color: '#f43f5e', marginBottom: '1rem', fontSize: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    <div style={{
                      ...
                      <span style={{ color: '#60a5fa', fontWeight: '600' }}>HEX</span>
                      <code style={{ color: '#e2e8f0', fontFamily: 'monospace' }}>{currentColor.hex}</code>
                    </div>
                    <div style={{
                      padding: '0.6rem 1rem',
                      ...
                      <span style={{ color: '#10b981', fontWeight: '600' }}>RGB</span>
                    ...
                    <div style={{
                      ...
                      <span style={{ color: '#8b5cf6', fontWeight: '600' }}>RGBA</span>
                      <code style={{ color: '#e2e8f0', fontFamily: 'monospace' }}>
                        rgba({currentColor.rgb.r}, {currentColor.rgb.g}, {currentColor.rgb.b}, {currentColor.rgb.a || 1})
                      </code>
                    </div>
                    <div style={{
                      padding: '0.6rem 1rem',
                      ...
                      <span style={{ color: '#f59e0b', fontWeight: '600' }}>CMYK</span>
                      <code style={{ color: '#e2e8f0', fontFamily: 'monospace' }}>
              ...
              <button
                type="button"
                onClick={() => {
                  const colorTexto = `[COLOR:${currentColor.hex}]\n` +
          ...
          <div>
            <h4 style={{ color: '#fb7185', marginBottom: '0.75rem', fontSize: '0.9rem' }}>
              ⭐ Colores rápidos
            </h4>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {['#ff0000', '#00ff00', '#0000ff', '#ffff00', '#ff00ff', '#00ffff', '#ff5733', '#c70039', '#900c3f', '#581845', '#1a1a2e', '#16213e'].map((color, idx) => (
                <button
                  key={idx}
                  type="button"
                  onClick={() => {
                  ...
                  style={{
                    ...
                    height: '40px',
                    background: color,
                    border: colorInput === color ? '3px solid white' : '2px solid rgba(255, 255, 255, 0.2)',
                    borderRadius: '8px',
                    cursor: 'pointer',
                    transition: 'all 0.2s'
                  }}
          ...
          <div style={{
            ...
            <h4 style={{ color: '#a78bfa', marginBottom: '1rem', fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {currentColor && (
              <button
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  color: coloresSeleccionados.includes(currentColor.hex) ? '#64748b' : '#c4b5fd',
                  cursor: coloresSeleccionados.includes(currentColor.hex) ? 'not-allowed' : 'pointer',
                  fontSize: '0.9rem',
                  fontWeight: '600',
                  marginBottom: '1rem',
                  display: 'flex',
                ...
                <div style={{ 
                  ...
                  {coloresSeleccionados.map((hex, idx) => (
                    ...
                    >
                      ...
                      <code style={{ color: '#e2e8f0', fontSize: '0.75rem' }}>{hex}</code>
                      <button
                        type="button"
                ...
                {coloresSeleccionados.length >= 2 && colorMezclado && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        {coloresSeleccionados.map((hex, idx) => (
                          <div key={idx} style={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            {idx < coloresSeleccionados.length - 1 && (
                              <span style={{ color: '#94a3b8', margin: '0 0.25rem', fontSize: '1.2rem' }}>+</span>
                            )}
                          </div>
                        ))}
                      </div>
                      ...
                      <code style={{ color: '#a7f3d0', fontFamily: 'monospace', fontSize: '0.85rem' }}>
                        {colorMezclado}
                      </code>
                    </div>
                  </div>
            ...
            {coloresSeleccionados.length === 0 && (
              <p style={{ color: '#94a3b8', fontSize: '0.8rem', textAlign: 'center', margin: 0 }}>
                Selecciona un color arriba y haz clic en "Añadir a la combinación"<br/>
