  ...
  return (
    ...
    >
        ...
        >
          ...
          {showCopyButton && (
            <button
              ...
              onMouseOver={(e) => {
                e.target.style.backgroundColor = darkMode ? '#333333' : '#e5e7eb';
              }}
              onMouseOut={(e) => {
      ...
      <div style={{ maxHeight, overflow: 'auto' }}>
        <SyntaxHighlighter
          ...
          customStyle={{
            ...
            padding: '12px',
            fontSize: '0.9rem',
            fontFamily: "'Fira Code', 'Consolas', 'Monaco', 'SF Mono', monospace",
            backgroundColor: darkMode ? '#1e1e1e' : '#ffffff',
          }}
          codeTagProps={{
            style: {
      ...
      {Object.keys(annotations).length > 0 && (
        ...
        >
          <div style={{ fontSize: '0.8rem', color: darkMode ? '#808080' : '#6b7280', marginBottom: '4px' }}>
            📝 Anotaciones:
          </div>
          {Object.entries(annotations).map(([line, comment]) => (
            ...
            >
              <span style={{ 
                color: errorLines.includes(parseInt(line)) ? '#ef4444' : '#3b82f6',
                fontWeight: '600',
              }}>
                L{line}:
              </span>
        ...
        {options.map((option, index) => {
          ...
          return (
            ...
            >
              <span style={{
                fontWeight: '600',
                color: isCorrect && showAnswer ? '#22c55e' : (isSelected ? '#3b82f6' : (darkMode ? '#9ca3af' : '#6b7280')),
                minWidth: '24px',
              }}>
                {letter})
              </span>
    ...
    <div className="code-complete">
      ...
      <div style={{ 
        ...
        {blanks.map((blank) => {
          ...
          const isCorrect = showAnswers && userValue.trim() === correctValue?.trim();
          const isWrong = showAnswers && userValue && userValue.trim() !== correctValue?.trim();
          
          return (
            <div key={blank.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{
                color: darkMode ? '#808080' : '#6b7280',
                fontWeight: '500',
                minWidth: '80px',
              ...
              <input
                type="text"
                value={userValue}
                onChange={(e) => onAnswerChange && onAnswerChange(blank.id, e.target.value)}
