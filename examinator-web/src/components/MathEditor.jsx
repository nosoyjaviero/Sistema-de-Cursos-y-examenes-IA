...
const MathEditor = forwardRef(({ value, onChange, placeholder = 'Escribe matemáticas...' }, ref) => {
    ...
    insertLatex: (latex) => {
      if (modoEdicion === 'codigo' || !mathLiveAvailable) {
        ...
        if (textarea) {
          ...
          const newValue = currentValue.slice(0, start) + processedLatex + currentValue.slice(end)
          onChange?.(newValue)
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const fallback = setTimeout(() => {
      if (!isMounted) {
        console.warn('MathLive custom element no detectado, habilitando de todos modos')
  ...
  const renderModeButtons = () => (
    <div style={{
      ...
      <button
        ...
        style={{
          padding: '0.5rem 1rem',
          background: modoEdicion === 'visual' 
            ? 'linear-gradient(135deg, #9333ea, #7c3aed)' 
            : 'rgba(147, 51, 234, 0.1)',
          color: modoEdicion === 'visual' ? '#fff' : '#c4b5fd',
          border: modoEdicion === 'visual' 
            ? '1px solid #a855f7' 
      ...
      <button
        ...
        style={{
          padding: '0.5rem 1rem',
          background: modoEdicion === 'codigo' 
            ? 'linear-gradient(135deg, #9333ea, #7c3aed)' 
            : 'rgba(147, 51, 234, 0.1)',
          color: modoEdicion === 'codigo' ? '#fff' : '#c4b5fd',
          border: modoEdicion === 'codigo' 
            ? '1px solid #a855f7' 
            : '1px solid rgba(147, 51, 234, 0.3)',
          borderRadius: '8px',
          cursor: 'pointer',
...
Puedes pegar LaTeX largo de ChatGPT:`}
      ...
      <details style={{ marginTop: '0.75rem' }}>
        ...
        <div style={{
          ...
          <div>
            <strong style={{ color: '#a5b4fc' }}>Fracciones:</strong><br/>
            <code>\frac{'{num}'}{'{den}'}</code><br/>
            <code>\dfrac{'{a}'}{'{b}'}</code> (grande)
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Exponentes/Subíndices:</strong><br/>
            <code>x^{'{2}'}</code> - exponente<br/>
            <code>a_{'{n}'}</code> - subíndice
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Raíces:</strong><br/>
            <code>\sqrt{'{x}'}</code><br/>
            <code>\sqrt[3]{'{x}'}</code> - cúbica
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Integrales:</strong><br/>
            <code>\int_{'{a}'}^{'{b}'} f(x)dx</code><br/>
            <code>\iint, \iiint</code>
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Límites:</strong><br/>
            <code>\lim_{'{x \\to a}'} f(x)</code><br/>
            <code>\lim_{'{n \\to \\infty}'}</code>
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Sumatorias:</strong><br/>
            <code>\sum_{'{i=1}'}^{'{n}'} a_i</code><br/>
            <code>\prod_{'{i=1}'}^{'{n}'}</code>
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Matrices:</strong><br/>
            <code>\begin{'{pmatrix}'} a & b \\ c & d \end{'{pmatrix}'}</code>
          </div>
          <div>
            <strong style={{ color: '#a5b4fc' }}>Griegos:</strong><br/>
            <code>\alpha \beta \gamma \delta</code><br/>
