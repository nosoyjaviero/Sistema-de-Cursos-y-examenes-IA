...
const ESTRUCTURAS_COMUNES = {
  ...
  acidos: [
    ...
    { nombre: 'Ácido cítrico', smiles: 'C(C(=O)O)(CC(=O)O)(CC(=O)O)O', emoji: '🍊' },
  ...
  aminoacidos: [
    ...
    { nombre: 'Fenilalanina (Phe)', smiles: 'c1ccc(cc1)CC(N)C(=O)O', emoji: 'F' },
    { nombre: 'Tirosina (Tyr)', smiles: 'c1cc(ccc1CC(C(=O)O)N)O', emoji: 'Y' },
    { nombre: 'Triptófano (Trp)', smiles: 'c1ccc2c(c1)c(cn2)CC(C(=O)O)N', emoji: 'W' },
    { nombre: 'Lisina (Lys)', smiles: 'C(CCN)CC(C(=O)O)N', emoji: 'K' },
    { nombre: 'Arginina (Arg)', smiles: 'C(CC(C(=O)O)N)CN=C(N)N', emoji: 'R' },
    { nombre: 'Histidina (His)', smiles: 'c1c([nH]cn1)CC(C(=O)O)N', emoji: 'H' },
  ...
  // ═══════════════════════════════════════════
  azucares: [
    { nombre: 'D-Glucosa', smiles: 'OC[C@H]1OC(O)[C@H](O)[C@@H](O)[C@@H]1O', emoji: '🍬' },
    { nombre: 'D-Fructosa', smiles: 'OC[C@H]1OC(O)(CO)[C@@H](O)[C@@H]1O', emoji: '🍯' },
    { nombre: 'D-Galactosa', smiles: 'OC[C@H]1OC(O)[C@H](O)[C@H](O)[C@H]1O', emoji: '🥛' },
    { nombre: 'D-Ribosa', smiles: 'OC[C@H]1OC(O)[C@H](O)[C@@H]1O', emoji: 'RNA' },
    { nombre: 'D-Desoxirribosa', smiles: 'OC[C@H]1OC(O)C[C@@H]1O', emoji: 'DNA' },
    { nombre: 'Sacarosa', smiles: 'OC[C@H]1OC(O[C@@H]2OC(CO)[C@@H](O)[C@H](O)[C@H]2O)[C@@H](O)[C@@H](O)[C@@H]1O', emoji: '🧁' },
    { nombre: 'Maltosa', smiles: 'OC[C@H]1OC(O[C@@H]2[C@@H](O)[C@H](O)[C@@H](O)OC2CO)[C@@H](O)[C@@H](O)[C@@H]1O', emoji: '🍺' },
  ],
  ...
  basesN: [
    ...
    { nombre: 'Guanina (G)', smiles: 'c1nc2c(n1)c(=O)[nH]c(n2)N', emoji: 'G🧬' },
    ...
    { nombre: 'ATP', smiles: 'c1nc(c2c(n1)n(cn2)[C@H]3[C@@H]([C@@H]([C@H](O3)COP(=O)([O-])OP(=O)([O-])OP(=O)([O-])[O-])O)O)N', emoji: '⚡' },
  ],
  ...
  moleculas: [
    ...
    { nombre: 'Colesterol', smiles: 'CC(C)CCCC(C)C1CCC2C1(CCC3C2CC=C4C3(CCC(C4)O)C)C', emoji: '🫀' },
  ],
  ...
  aromaticos: [
    ...
    { nombre: 'Fenol', smiles: 'Oc1ccccc1', emoji: '⬡-OH' },
    { nombre: 'Anilina', smiles: 'Nc1ccccc1', emoji: '⬡-NH₂' },
    { nombre: 'Nitrobenceno', smiles: 'c1ccc(cc1)[N+](=O)[O-]', emoji: '⬡-NO₂' },
    ...
    { nombre: 'Benzoato de metilo', smiles: 'COC(=O)c1ccccc1', emoji: '⬡-COOMe' },
    ...
    { nombre: 'TNT', smiles: 'Cc1c(cc(cc1[N+](=O)[O-])[N+](=O)[O-])[N+](=O)[O-]', emoji: '💥' },
  ],
  ...
  azufrados: [
    ...
    { nombre: 'Sulfonamida', smiles: 'c1ccc(cc1)S(=O)(=O)N', emoji: '⬡-SO₂NH₂' },
  ],
}

const ChemEditor = ({ value, onChange, placeholder = 'Estructura química...', readOnly = false }) => {
  ...
  useEffect(() => {
    ...
    const partes = inputSmiles.split(';')
      .map(s => s.trim())
      .filter(s => s.length > 0)
    
    setEstructuras(partes.map((smiles, idx) => ({
  ...
  const drawEstructura = useCallback((estructura, svgElement) => {
    ...
    try {
      drawerRef.current.draw(
        ...
        'dark',
        () => {
          setEstructuras(prev => prev.map(e => 
        ...
        (err) => {
          ...
          setEstructuras(prev => prev.map(e => 
            e.id === estructura.id ? { ...e, drawn: false, error: 'SMILES inválido' } : e
          ))
          setErrores(prev => ({ ...prev, [estructura.id]: 'SMILES inválido' }))
        }
      )
    } catch (e) {
  ...
  return (
    <div style={{
            ...
            <div style={{ padding: '0.75rem' }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', minWidth: '70px' }}>🔥 Hidrocarburos:</span>
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: '4px',
                        border: 'none',
                        background: tabActiva === tab.key ? 'rgba(16, 185, 129, 0.4)' : 'rgba(100, 116, 139, 0.15)',
                ...
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', minWidth: '70px' }}>🔗 Funcionales:</span>
                  {[
                ...
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', minWidth: '70px' }}>⚗️ Ácidos/Bases:</span>
                  {[
                ...
                {/* Fila 4: Bioquímica */}
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', minWidth: '70px' }}>🧬 Bio:</span>
                  {[
                ...
                {/* Fila 5: General */}
                <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.7rem', color: '#94a3b8', minWidth: '70px' }}>⚛️ General:</span>
                  {[
      ...
      {estructuras.length > 0 ? (
        <div style={{
          display: 'grid',
          gridTemplateColumns: estructuras.length === 1 
            ? '1fr' 
            ...
            >
              ...
              <div style={{
                ...
                <span style={{ 
                  ...
                  {est.smiles.length > 30 ? est.smiles.slice(0, 30) + '...' : est.smiles}
                </span>
                {!readOnly && (
                  <button
                    type="button"
              ...
              <div style={{
                ...
                {est.error ? (
                  <div style={{ color: '#fca5a5', textAlign: 'center', fontSize: '0.85rem' }}>
                    ⚠️ {est.error}<br/>
      ...
      {!readOnly && (
        <div style={{
          ...
          <strong>📖 Referencia SMILES:</strong>{' '}
          <code>c1ccccc1</code>=Benceno, <code>C</code>=Carbono, <code>=</code>=Doble enlace, 
          <code>#</code>=Triple, <code>()</code>=Ramificación, <code>[]</code>=Átomo especial
        </div>
      )}
    </div>
  )
